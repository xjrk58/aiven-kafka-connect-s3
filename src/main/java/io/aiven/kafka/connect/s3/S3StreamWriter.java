...
import static io.aiven.kafka.connect.common.config.FormatterUtils.formatKafkaOffset;
...
public class S3StreamWriter {
    ...
    private final S3SyncConfig config;

    private final AmazonS3 s3Client;

    private final Map<TopicPartition, OutputStream> streams;

    public S3StreamWriter(final S3SyncConfig config) {
        super();
        ...
        if (Objects.isNull(awsEndPOntConfig)) {
            s3ClientBuilder.withRegion(config.getRegion());
        } else {
            s3ClientBuilder
    ...
    private AwsClientBuilder.EndpointConfiguration newEndpointConfiguration(final S3SyncConfig config) {
        return Objects.nonNull(config.getAwsEndPoint())
            ? new AwsClientBuilder.EndpointConfiguration(config.getAwsEndPoint(), config.getRegion().getName())
            : null;
    }
    ...
    private OutputStream newStreamFor(final SinkRecord record) {
        ...
        final var awsOutputStream = new S3OutputStream(s3Client, config.getBucketName(), fullKey);
        try {
